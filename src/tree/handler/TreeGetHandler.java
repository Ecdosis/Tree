...
public class TreeGetHandler extends GetHandler
{
        ...
        HttpServletResponse response, String urn ) throws TreeException
    {
        ...
        while ( iter.hasNext() )
        {
            ...
            if ( values.length>0 )
            {
                if ( key.equals(Params.DOCID) )
                    docid = values[0];
                else if ( key.equals(Params.FONT) )
                    font = values[0];
                else if ( key.equals(Params.TREEGROWS) )
                    treeGrows = values[0];
                else if ( key.equals(Params.TREESTYLE) )
                    treeStyle = values[0];
                else if ( key.equals(Params.USEBRANCHLENGTHS) )
                    useBranchLengths = Boolean.parseBoolean(values[0]);
            }
        }
        if ( docid != null )
        {
